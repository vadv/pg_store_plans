...
norm_yylex(char *str, core_YYSTYPE *yylval, YYLTYPE *yylloc, core_yyscan_t yyscanner)
{
	...
	PG_CATCH();
	{
		 ...
		 * halfway. Just ignore the rest of this query even if there might be
		 * other reasons for parsing to fail.
		 */
		FlushErrorState();
		return -1;
	}
	...
	/*
	 * '?' alone is assumed to be an IDENT.  If there's a real operator '?',
	 * this should be confused but there's hardly be.
	 */
	...
	/*
	 * Replace tokens with '=' if the operator is consists of two or more
	 * opchars only. Assuming that opchars do not compose a token with
	 * non-opchars, check the first char only is sufficient.
	 */
...
normalize_expr(char *expr, bool preserve_space)
{
	...
	for (;;)
	{
		...
		if (lastloc >= 0)
		{
			 ...
			 * in the column "query". Normalization is basically useless in
			 * the version but still usefull to match utility commands so
			 * follow the behavior change.
			 */
			else if (lasttok == ';')
			{
				/* Just do nothing */
			 ...
			 * reading point. Although this function's output does not need
			 * any validity as a statement or an expression, spaces are added
			 * where it should be to keep some extent of sanity.  If
			 * readability is more important than uniqueness, preserve_space
			 * adds one space for each existent whitespace.
			 */
			if (tok > 0 &&
				i2 < start &&
				(preserve_space ||
		...
		if (IS_CONST(tok))
		{
			...
			/*
			 * Negative values may be surrounded with parens by the deparser.
			 * Mask involving them.
			 */
			if (lasttok == '(' && tok == ')')
			{
...
json_ofstart(void *state, char *fname, bool isnull)
{
	...
	/*
	 * We intentionally let some property names not have a short name. Use
	 * long name for the cases.
	 */
	if (!p || !p->longname)
		fn = fname;
...
xml_ofstart(void *state, char *fname, bool isnull)
{
	...
	/*
	 * save current process context There's no problem if P_Plan appears
	 * recursively.
	 */
