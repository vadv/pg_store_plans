...
static void print_prop(StringInfo s, char *prepstr, const char *prop, int leve,
                       int exind);
static void print_prop_if_exists(StringInfo s, char *prepstr, const char *prop,
                                 int leve, int exind);
static void print_prop_if_nz(StringInfo s, char *prepstr, const char *prop,
                             int leve, int exind);
...
#define TEXT_INDENT_DETAILS(l, e) (TEXT_INDENT_BASE(l, e) + ((l < 2) ? 2 : 6))

static void print_obj_name0(StringInfo s, const char *obj_name,
                            const char *schema_name, const char *alias) {
...
static void print_obj_name(pgspParserContext *ctx) {
...
static void print_prop(StringInfo s, char *prepstr, const char *prop, int level,
                       int exind) {
    if (level > 0) {
...
static void print_propstr_if_exists(StringInfo s, char *prepstr,
...
static void print_groupingsets_if_exists(StringInfo s, List *gss, int level,
                                         int exind) {
    ListCell *lc;
...
static void print_current_node(pgspParserContext *ctx) {
    ...
    if (!ISZERO(v->startup_cost) && !ISZERO(v->total_cost) &&
        HASSTRING(v->plan_rows) && HASSTRING(v->plan_width)) {
    ...
    else if (HASSTRING(v->actual_rows) && HASSTRING(v->actual_loops) &&
    ...
    print_prop_if_exists(s, "Workers Planned: ", v->workers_planned, level,
                         exind);
    print_prop_if_exists(s, "Workers Launched: ", v->workers_launched, level,
                         exind);

    if (HASSTRING(v->sampling_method)) {
        appendStringInfoString(s, "\n");
        appendStringInfoSpaces(s, TEXT_INDENT_DETAILS(level, exind));
        appendStringInfo(s, "Sampling: %s (%s)", v->sampling_method,
    ...
    if (HASSTRING(v->sort_method)) {
        ...
        if (HASSTRING(v->sort_space_type) && HASSTRING(v->sort_space_used)) {
            appendStringInfoString(s, "  ");
    ...
    print_prop_if_nz(s, "Rows Removed by Filter: ", v->filter_removed, level,
                     exind);
    print_prop_if_nz(s, "Rows Removed by Index Recheck: ", v->idxrchk_removed,
                     level, exind);
    print_prop_if_nz(s, "Rows Removed by Join Filter: ", v->joinfilt_removed,
                     level, exind);

    if (HASSTRING(v->exact_heap_blks) || HASSTRING(v->lossy_heap_blks)) {
        appendStringInfoString(s, "\n");
        appendStringInfoSpaces(s, TEXT_INDENT_DETAILS(level, exind));
    ...
    print_prop_if_exists(s, "Conflict Resolution: ", v->conflict_resolution,
                         level, exind);
    print_propstr_if_exists(s, "Conflict Arbiter Indexes: ",
                            v->conflict_arbiter_indexes, level, exind);
    print_prop_if_exists(s, "Tuples Inserted: ", v->tuples_inserted, level,
                         exind);
    print_prop_if_exists(s, "Conflicting Tuples: ", v->conflicting_tuples,
                         level, exind);

    if (!ISZERO(v->shared_hit_blks) || !ISZERO(v->shared_read_blks) ||
        !ISZERO(v->shared_dirtied_blks) || !ISZERO(v->shared_written_blks)) {
        appendStringInfoString(s, "\n");
        appendStringInfoSpaces(s, TEXT_INDENT_DETAILS(level, exind));
    ...
    if (!ISZERO(v->local_hit_blks) || !ISZERO(v->local_read_blks) ||
        !ISZERO(v->local_dirtied_blks) || !ISZERO(v->local_written_blks)) {
        if (comma)
            appendStringInfoString(s, ", ");
        else {
            appendStringInfoSpaces(s, TEXT_INDENT_DETAILS(level, exind));
    ...
    if (!ISZERO(v->temp_read_blks) || !ISZERO(v->temp_written_blks)) {
        if (comma)
            appendStringInfoString(s, ", ");
        else {
            appendStringInfoSpaces(s, TEXT_INDENT_DETAILS(level, exind));
    ...
    if (!ISZERO(v->io_read_time) || !ISZERO(v->io_write_time)) {
...
static void json_text_objend(void *state) {
    ...
    if (bms_is_member(ctx->level - 1, ctx->plan_levels)) {
        ...
        v->target_tables =
            lappend(v->target_tables, pstrdup(ctx->work_str->data));
        resetStringInfo(ctx->work_str);
        ...
        /* Move working grouping set into nodevals */
        ctx->nodevals->grouping_sets = lappend(v->grouping_sets, ctx->tmp_gset);
        ctx->tmp_gset = NULL;
    }
    ...
    if (ctx->current_list == P_GroupSets) {
        ...
        if (ctx->wlist_level == 3) {
            ...
            if (v->group_key->data[0]) {
                ctx->tmp_gset->group_keys = lappend(
                    ctx->tmp_gset->group_keys, pstrdup(v->group_key->data));
            } else if (v->hash_key->data[0]) {
                ctx->tmp_gset->group_keys = lappend(ctx->tmp_gset->group_keys,
...
static void json_text_ofstart(void *state, char *fname, bool isnull) {
    ...
    if (!p) {
        ereport(DEBUG2,
                (errmsg("Short JSON parser encoutered unknown field name: "
                        "\"%s\", skipped.",
                        fname),
...
static void json_text_ofend(void *state, char *fname, bool isnull) {
    ...
    if (HASSTRING(v->plan_time) || HASSTRING(v->exec_time)) {
...
static void json_text_scalar(void *state, char *token,
