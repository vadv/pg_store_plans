 ...
 *
 * pg_store_plans.c
 *		Take statistics of plan selection across a whole database
 *cluster.
 *
...
static const struct config_enum_entry plan_formats[] = {
    { "raw", PLAN_FORMAT_RAW, false },   { "text", PLAN_FORMAT_TEXT, false },
    { "json", PLAN_FORMAT_JSON, false }, { "yaml", PLAN_FORMAT_YAML, false },
    { "xml", PLAN_FORMAT_XML, false },   { NULL, 0, false }
};
...
static StatEntry *entry_alloc(EntryKey *key, const char *plan, int plan_len,
                              bool sticky);
...
void _PG_init(void) {
    ...
    DefineCustomIntVariable(
        "pg_store_plans.max",
        "Sets the maximum number of plans tracked by pg_store_plans.", NULL,
        &store_size, 1000, 100, INT_MAX, PGC_POSTMASTER, 0, NULL, NULL, NULL);

    DefineCustomEnumVariable(
        "pg_store_plans.track",
        "Selects which plans are tracked by pg_store_plans.", NULL,
        &track_level, TRACK_LEVEL_TOP, track_options, PGC_SUSET, 0, NULL, NULL,
        NULL);

    DefineCustomEnumVariable("pg_store_plans.plan_format",
                             "Selects which format to be appied for plan "
                             "representation in pg_store_plans.",
                             NULL, &plan_format, PLAN_FORMAT_TEXT, plan_formats,
                             PGC_USERSET, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "pg_store_plans.min_duration", "Minimum duration to record plan.", NULL,
        &min_duration, 0, 0, INT_MAX, PGC_SUSET, GUC_UNIT_MS, NULL, NULL, NULL);

    DefineCustomIntVariable("pg_store_plans.slow_statement_duration",
                            "Unconditional record plan of slow statement.",
                            NULL, &slow_statement_duration, 0, 0, INT_MAX,
                            PGC_SUSET, GUC_UNIT_MS, NULL, NULL, NULL);

    DefineCustomBoolVariable(
        "pg_store_plans.save",
        "Save pg_store_plans statistics across server shutdowns.", NULL,
        &dump_on_shutdown, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable(
        "pg_store_plans.store_last_plan", "Always update text of stored plan.",
        NULL, &store_last_plan, false, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable(
        "pg_store_plans.log_analyze", "Use EXPLAIN ANALYZE for plan logging.",
        NULL, &log_analyze, false, PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable("pg_store_plans.log_buffers", "Log buffer usage.",
                             NULL, &log_buffers, false, PGC_SUSET, 0, NULL,
                             NULL, NULL);

    DefineCustomBoolVariable("pg_store_plans.log_timing", "Log timings.", NULL,
                             &log_timing, true, PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable("pg_store_plans.log_triggers",
                             "Log trigger trace.", NULL, &log_triggers, false,
                             PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable(
        "pg_store_plans.log_verbose", "Set VERBOSE for EXPLAIN on logging.",
        NULL, &log_verbose, false, PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomRealVariable(
        "pg_store_plans.sample_rate", "Fraction of queries to process.", NULL,
        &sample_rate, 1.0, 0.0, 1.0, PGC_SUSET, 0, NULL, NULL, NULL);
...
static void pgsp_shmem_startup(void) {
    ...
    shared_state =
        ShmemInitStruct("pg_store_plans", sizeof(SharedState), &found);
    ...
    hash_table = ShmemInitHash("pg_store_plans hash", store_size, store_size,
    ...
    if (fread(&header, sizeof(uint32), 1, file) != 1 ||
        header != PGSP_FILE_HEADER || fread(&num, sizeof(int32), 1, file) != 1)
        goto error;
    ...
    for (i = 0; i < num; i++) {
        ...
        if (temp.plan_len >= plan_size)
            temp.plan_len = pg_encoding_mbcliplen(temp.encoding, buffer,
                                                  temp.plan_len, plan_size - 1);
 ...
 */
static void pgsp_ExecutorStart(QueryDesc *queryDesc, int eflags) {
    if (log_analyze && (eflags & EXEC_FLAG_EXPLAIN_ONLY) == 0) {
        queryDesc->instrument_options |= (log_timing ? INSTRUMENT_TIMER : 0) |
                                         (log_timing ? 0 : INSTRUMENT_ROWS) |
                                         (log_buffers ? INSTRUMENT_BUFFERS : 0);
    }

    current_query_sampled =
        (random() < sample_rate * ((double)MAX_RANDOM_VALUE + 1));
        ...
        if ((pgsp_enabled() &&
            ...
            (slow_statement_duration > 0 && nested_level == 0 &&
             ...
             queryDesc->totaltime->total >=
                 (double)slow_statement_duration / 1000.0)) {
            ...
            store_entry(es_str->data, 0, queryDesc->plannedstmt->queryId,
...
static void store_entry(char *plan, uint32 queryId, queryid_t queryId_pgss,
    ...
    if (store_last_plan) {
        ...
        if (plan_len >= shared_state->plan_size)
            plan_len =
                pg_encoding_mbcliplen(GetDatabaseEncoding(), shorten_plan,
                                      plan_len, shared_state->plan_size - 1);
    }
    ...
    if (e->counters.calls == 1) {
        ...
        e->counters.mean_time += (total_time - old_mean) / e->counters.calls;
...
Datum pg_store_plans_reset(PG_FUNCTION_ARGS) {
    if (!shared_state || !hash_table)
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg("pg_store_plans must be loaded via "
...
Datum pg_store_plans(PG_FUNCTION_ARGS) {
    ...
    while ((entry = hash_seq_search(&hash_seq)) != NULL) {
        ...
        if (is_allowed_role || entry->key.userid == userid) {
            ...
            estr = (char *)pg_do_encoding_conversion(
                (unsigned char *)pstr, strlen(pstr), entry->encoding,
                GetDatabaseEncoding());
...
static StatEntry *entry_alloc(EntryKey *key, const char *plan, int plan_len,
    ...
    if (!found) {
        ...
        entry->counters.usage =
            sticky ? shared_state->cur_median_usage : USAGE_INIT;
